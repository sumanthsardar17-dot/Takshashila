...
const C = {
  ...
  glassBorder: "rgba(201,164,50,0.18)",
  cardBg: "rgba(7,30,51,0.85)",
};
...
const PROMPTS = [
  "Explain pharmacokinetics simply","How do beta-blockers work?",
  "What is bioavailability?","Explain first pass metabolism",
  "Types of drug interactions","What is the blood-brain barrier?",
  "Explain ADME with examples","What is GMP in pharma?",
  "Difference B.Pharm vs Pharm.D?","Zero order vs first order kinetics",
];
...
═══════════════════════════════════════════════════ */
function SubjectCard({topic, accent, i}) {
...
/* ═══════════════════════════════════════════════════
   AI TUTOR
═══════════════════════════════════════════════════ */
function AiTutor({program}) {
  const prog = program ? PROGRAMS[program] : null;
  const accent = prog?.glow || C.gold1;
  const [msgs,setMsgs] = useState([{
    role:"assistant",
    content:`🙏 Namaste! I'm the **Takshshila AI Tutor** — your expert pharmacy guide.\n\nI can help with ${prog?prog.fullName:"B.Pharm, D.Pharm & Pharm.D"} — drug mechanisms, pharmacokinetics, clinical pharmacy, calculations, mnemonics, and exam preparation.\n\nWhat would you like to learn today?`,
  }]);
    ...
    try{
      const r=await fetch("https://api.anthropic.com/v1/messages",{
        ...
        body:JSON.stringify({
          ...
          system:`You are the Takshshila Pharmacy Knowledge Hub AI Tutor — an expert, enthusiastic pharmacy educator. You help Indian pharmacy students in ${prog?prog.fullName:"B.Pharm, D.Pharm, and Pharm.D"}. Explain concepts clearly with clinical examples, helpful mnemonics, and exam tips. Use structured formatting with headings and bullet points where helpful. Reference Indian pharmacy curriculum (PCI guidelines) where relevant. Be warm, encouraging and thorough.`,
          messages:next.map(m=>({role:m.role,content:m.content})),
        }),
      });
      const d=await r.json();
      const reply=d.content?.map(c=>c.text).join("")||"Sorry, couldn't get a response.";
      setMsgs([...next,{role:"assistant",content:reply}]);
    }catch{
      setMsgs([...next,{role:"assistant",content:"⚠️ Connection error. Please try again."}]);
    }
    setLoading(false);
  };
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100%",background:C.bg0}}>
      {/* Quick prompts */}
      <div style={{padding:"10px 14px",borderBottom:`1px solid ${C.dim}44`,overflowX:"auto",display:"flex",gap:7,flexShrink:0,background:C.bg1}}>
        {PROMPTS.map(p=>(
          <button key={p} onClick={()=>send(p)} style={{
            background:"none",border:`1px solid ${accent}44`,color:accent+"cc",
            borderRadius:20,padding:"5px 13px",fontSize:11,cursor:"pointer",
            whiteSpace:"nowrap",fontFamily:"'Nunito',sans-serif",fontWeight:600,
            transition:"all 0.2s",flexShrink:0,
          }}
          onMouseEnter={e=>{e.target.style.background=accent+"18";e.target.style.borderColor=accent;}}
          onMouseLeave={e=>{e.target.style.background="none";e.target.style.borderColor=accent+"44";}}
          >{p}</button>
        ))}
      </div>
      {/* Messages */}
      <div style={{flex:1,overflowY:"auto",padding:"16px 14px",display:"flex",flexDirection:"column",gap:14}}>
        {msgs.map((m,i)=>(
          <div key={i} style={{display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start",gap:10,animation:"fadeUp 0.3s ease both"}}>
            {m.role==="assistant" && (
              <div style={{width:34,height:34,borderRadius:"50%",background:`linear-gradient(135deg,${C.teal1},${C.teal2})`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0,boxShadow:`0 0 12px ${C.teal2}44`,marginTop:2}}>🏛</div>
            )}
            <div style={{
              maxWidth:"76%",padding:"12px 16px",
              borderRadius:m.role==="user"?"18px 4px 18px 18px":"4px 18px 18px 18px",
              background:m.role==="user"
                ?`linear-gradient(135deg,${C.teal1},${C.teal2})`
                :C.cardBg,
              color:C.white,fontSize:13.5,lineHeight:1.75,
              border:m.role==="assistant"?`1px solid ${C.dim}88`:"none",
              boxShadow:m.role==="assistant"?`0 2px 16px rgba(0,0,0,0.3)`:`0 2px 12px ${C.teal2}33`,
              whiteSpace:"pre-wrap",fontFamily:"'Nunito',sans-serif",
            }}>{m.content}</div>
            {m.role==="user" && (
              <div style={{width:34,height:34,borderRadius:"50%",background:`linear-gradient(135deg,${C.gold0},${C.gold1})`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,flexShrink:0,marginTop:2}}>👤</div>
            )}
          </div>
        ))}
        {loading && (
          <div style={{display:"flex",gap:10,alignItems:"flex-start"}}>
            <div style={{width:34,height:34,borderRadius:"50%",background:`linear-gradient(135deg,${C.teal1},${C.teal2})`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16}}>🏛</div>
            <div style={{background:C.cardBg,border:`1px solid ${C.dim}88`,borderRadius:"4px 18px 18px 18px",padding:"12px 18px",display:"flex",gap:6,alignItems:"center"}}>
              {[0,1,2].map(i=>(
                <div key={i} style={{width:8,height:8,borderRadius:"50%",background:accent,animation:`bounce 1.2s ease ${i*0.18}s infinite`,boxShadow:`0 0 8px ${accent}`}} />
              ))}
            </div>
          </div>
        )}
      ...
      {/* Input */}
      <div style={{padding:"12px 14px",borderTop:`1px solid ${C.dim}44`,display:"flex",gap:10,background:C.bg1,flexShrink:0}}>
        <input value={input} onChange={e=>setInput(e.target.value)}
          onKeyDown={e=>e.key==="Enter"&&!e.shiftKey&&send()}
          placeholder="Ask any pharmacy question..."
          style={{
            flex:1,background:C.bg2,
            border:`1px solid ${C.dim}88`,
            borderRadius:28,padding:"11px 20px",
            color:C.white,fontSize:14,outline:"none",
            fontFamily:"'Nunito',sans-serif",
            transition:"border-color 0.2s",
          }}
          onFocus={e=>e.target.style.borderColor=accent}
          onBlur={e=>e.target.style.borderColor=C.dim+"88"}
        />
        <button onClick={()=>send()} disabled={loading||!input.trim()} style={{
          background:input.trim()&&!loading?`linear-gradient(135deg,${C.gold0},${C.gold2})`:`${C.dim}55`,
          border:"none",borderRadius:"50%",width:46,height:46,
          cursor:input.trim()&&!loading?"pointer":"not-allowed",
          fontSize:18,flexShrink:0,transition:"all 0.25s",
          boxShadow:input.trim()&&!loading?`0 4px 16px ${C.gold1}44`:"none",
        }}>➤</button>
      </div>
    </div>
  );
...
export default function App() {
  ...
  const goProg=(key)=>{setProg(key);setProgTab("subjects");setPage("program");};
    ...
    <div style={{fontFamily:"'Nunito',sans-serif",background:C.bg0,minHeight:"100vh",color:C.white,overflowX:"hidden"}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .prog-card{transition:all 0.3s!important;}
        .prog-card:hover{transform:translateY(-4px)!important;box-shadow:0 16px 48px rgba(0,0,0,0.5)!important;}
      ...
      <nav style={{
        ...
        <div style={{display:"flex",alignItems:"center",gap:11,cursor:"pointer"}} onClick={()=>setPage("home")}>
          <div style={{
            ...
            display:"flex",alignItems:"center",justifyContent:"center",
            fontSize:18,boxShadow:`0 0 16px ${C.teal2}66`,
        ...
        <div style={{display:"flex",gap:2,alignItems:"center"}}>
          {[{id:"home",label:"Home"},{id:"bpharm",label:"B.Pharm",prog:true},{id:"dpharm",label:"D.Pharm",prog:true},{id:"pharmd",label:"Pharm.D",prog:true},{id:"ai-tutor",label:"🤖 AI Tutor"}].map(item=>{
        ...
        <div>
          ...
          <div style={{position:"relative",overflow:"hidden",minHeight:440,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"50px 24px 40px",
            ...
            <div style={{
              ...
              <div style={{fontSize:10,fontWeight:900,color:C.bg0,fontFamily:"'Nunito',sans-serif",lineHeight:1.4}}>🔔 RECRUITING<br/>PHARMACY EXPERTS!</div>
            ...
            <div style={{position:"relative",zIndex:5,textAlign:"center",animation:"fadeUp 0.7s ease both"}}>
              ...
              <p style={{color:C.muted,fontSize:14,maxWidth:460,margin:"0 auto 28px",lineHeight:1.8,fontWeight:600}}>
                Free pharmacy education for <span style={{color:C.gold1}}>B.Pharm</span>, <span style={{color:C.teal2}}>D.Pharm</span> & <span style={{color:"#4a90d9"}}>Pharm.D</span> — with an AI tutor powered by Claude.
              </p>
              <div style={{display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap"}}>
                ...
                onMouseEnter={e=>{e.currentTarget.style.background=C.gold1+"18";e.currentTarget.style.borderColor=C.gold2;}}
                onMouseLeave={e=>{e.currentTarget.style.background="none";e.currentTarget.style.borderColor=C.gold1+"88";}}
          ...
          <div style={{background:`linear-gradient(90deg,${C.teal0},${C.bg2},${C.teal0})`,borderTop:`1px solid ${C.gold1}22`,borderBottom:`1px solid ${C.gold1}22`,padding:"18px 20px",display:"flex",justifyContent:"space-around",flexWrap:"wrap",gap:12}}>
            {[["3","Programs","🎓"],["100+","Topics","📚"],["Free","Forever","🆓"],["AI","Powered","🤖"],["24/7","Support","⚡"]].map(([v,l,ic],i)=>(
              <div key={l} style={{textAlign:"center",animation:`fadeUp 0.4s ease ${i*0.08}s both`}}>
          ...
          <div style={{padding:"28px 18px 10px"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              {Object.entries(PROGRAMS).map(([key,pr],i)=>(
                  ...
                  <div style={{padding:"18px 18px",display:"flex",alignItems:"center",gap:16}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      <div style={{display:"flex",alignItems:"center",gap:6,marginTop:8}}>
                        <div style={{fontSize:11,color:C.muted,fontWeight:600}}>{pr.subjects.reduce((a,s)=>a+s.topics.length,0)} subjects</div>
          ...
          <div style={{margin:"24px 18px",background:`linear-gradient(135deg,${C.teal0},${C.bg2})`,border:`1px solid ${C.teal2}44`,borderRadius:20,padding:"22px 18px",position:"relative",overflow:"hidden",animation:"fadeUp 0.5s ease 0.4s both"}}>
            ...
            <h3 style={{fontFamily:"'Cinzel',serif",fontWeight:900,fontSize:17,color:C.white,marginBottom:8}}>Meet Your AI Tutor</h3>
            <p style={{color:C.muted,fontSize:13,lineHeight:1.7,marginBottom:16,fontWeight:600}}>Ask any pharmacy question — drug mechanisms, calculations, clinical cases, exam prep — and get instant expert answers powered by Claude AI.</p>
            <button onClick={()=>setPage("ai-tutor")} style={{
              background:`linear-gradient(135deg,${C.teal1},${C.teal2})`,
          ...
          <div style={{padding:"0 18px 16px"}}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
              <div style={{width:3,height:28,background:`linear-gradient(180deg,${C.gold2},${C.teal2})`,borderRadius:2}} />
              <h2 style={{fontFamily:"'Cinzel',serif",fontWeight:900,fontSize:16,color:C.gold2,letterSpacing:1.5}}>WHY TAKSHSHILA?</h2>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              {[
                ["🆓","100% Free","No subscription fees ever — quality pharmacy education for every student"],
                ["🤖","AI Tutor","Claude-powered expert guide available 24/7 for instant answers"],
      ...
      {page==="program" && p && (
        <div style={{animation:"fadeIn 0.35s ease both"}}>
          ...
          <div style={{
            ...
            <button onClick={()=>setPage("home")} style={{background:"none",border:"none",color:C.muted,cursor:"pointer",fontSize:13,marginBottom:14,fontFamily:"'Nunito',sans-serif",fontWeight:700,display:"flex",alignItems:"center",gap:4}}>
              ← Back to Hub
            </button>
            ...
            <div style={{padding:"22px 18px",animation:"fadeUp 0.4s ease both"}}>
              {p.subjects.map((sem,si)=>(
                <div key={sem.sem} style={{marginBottom:28}}>
                  ...
                  {sem.topics.map((t,ti)=><SubjectCard key={t.name} topic={t} accent={p.glow} i={ti} />)}
                </div>
              ))}
              {/* CTA */}
              <div style={{background:`linear-gradient(135deg,${p.glow}15,${C.teal0})`,border:`1px solid ${p.glow}33`,borderRadius:16,padding:"22px 18px",textAlign:"center",marginTop:8}}>
                ...
                <p style={{color:C.muted,fontSize:13,marginBottom:14,fontWeight:600,lineHeight:1.65}}>Ask our AI Tutor for instant explanations, mnemonics & exam tips.</p>
                <button onClick={()=>setProgTab("tutor")} style={{background:p.gradient,border:"none",color:C.white,padding:"11px 26px",borderRadius:24,fontWeight:900,fontSize:13,cursor:"pointer",fontFamily:"'Nunito',sans-serif",boxShadow:`0 6px 20px ${p.glow}44`}}>
                  Open AI Tutor →
                </button>
              </div>
            </div>
      ...
      {page==="ai-tutor" && (
        <div style={{animation:"fadeIn 0.3s ease both"}}>
          <div style={{padding:"18px 20px 14px",borderBottom:`1px solid ${C.dim}44`,background:`linear-gradient(135deg,${C.teal0},${C.bg2})`}}>
            <button onClick={()=>setPage("home")} style={{background:"none",border:"none",color:C.muted,cursor:"pointer",fontSize:13,marginBottom:10,fontFamily:"'Nunito',sans-serif",fontWeight:700}}>
              ← Back to Hub
            </button>
            <div style={{display:"flex",alignItems:"center",gap:12}}>
              ...
              <div>
                ...
                <p style={{color:C.muted,fontSize:11.5,marginTop:2,fontWeight:600}}>Powered by Claude · Expert answers for all pharmacy programs</p>
              </div>
            </div>
          </div>
...
function clamp(min,mid,max){ return Math.max(min,Math.min(max,mid)); }
